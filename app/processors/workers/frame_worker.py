...
def _helper_warp_image_with_tps_opencv(image_tensor_chw_rgb: torch.Tensor,
                                       source_landmarks_xy: np.ndarray,
                                       target_landmarks_tps_xy: np.ndarray,
                                       regularization_alpha: float = 0.01) -> torch.Tensor:
    """
    Warps an image using Thin Plate Splines (TPS) to map source_landmarks_xy to target_landmarks_tps_xy.
    Helper function intended for use within frame_worker.py.

    Args:
        image_tensor_chw_rgb: PyTorch tensor (C, H, W), RGB, values typically 0-255 (uint8 or float).
                              If float and max val <= 1.0, it's assumed to be in 0-1 range.
        source_landmarks_xy: NumPy array of shape (N, 2) for landmarks on the input image_tensor_chw_rgb.
                             Coordinates are relative to the frame of image_tensor_chw_rgb.
        target_landmarks_tps_xy: NumPy array of shape (N, 2) for target landmark positions for TPS.
                                 Coordinates are relative to the frame of image_tensor_chw_rgb.
        regularization_alpha: Regularization parameter for TPS.

    Returns:
        PyTorch tensor (C, H, W), RGB, same dtype and range convention as input,
        representing the warped image.
    """
    if not isinstance(image_tensor_chw_rgb, torch.Tensor):
        raise TypeError("image_tensor_chw_rgb must be a PyTorch tensor.")
    if not isinstance(source_landmarks_xy, np.ndarray) or source_landmarks_xy.ndim != 2 or source_landmarks_xy.shape[1] != 2:
        raise TypeError("source_landmarks_xy must be a NumPy array of shape (N, 2).")
    if not isinstance(target_landmarks_tps_xy, np.ndarray) or target_landmarks_tps_xy.ndim != 2 or target_landmarks_tps_xy.shape[1] != 2:
        raise TypeError("target_landmarks_tps_xy must be a NumPy array of shape (N, 2).")
    if source_landmarks_xy.shape != target_landmarks_tps_xy.shape:
        raise ValueError("source_landmarks_xy and target_landmarks_tps_xy must have the same shape.")

    device = image_tensor_chw_rgb.device
    original_dtype = image_tensor_chw_rgb.dtype
    is_float_0_1_range = (original_dtype == torch.float32 or original_dtype == torch.float64) and torch.max(image_tensor_chw_rgb) <= 1.0

    # 1. Convert PyTorch tensor to OpenCV format (HWC, BGR, 0-255 uint8)
    # Ensure tensor is on CPU before .numpy()
    numpy_image_hwc_rgb = image_tensor_chw_rgb.cpu().permute(1, 2, 0).numpy()

    if is_float_0_1_range:
        numpy_image_hwc_rgb = (numpy_image_hwc_rgb * 255)

    if not numpy_image_hwc_rgb.dtype == np.uint8:
        numpy_image_hwc_rgb = numpy_image_hwc_rgb.astype(np.uint8)

    if numpy_image_hwc_rgb.shape[2] == 4: # RGBA
        numpy_image_hwc_bgr = cv2.cvtColor(numpy_image_hwc_rgb, cv2.COLOR_RGBA2BGRA)
    elif numpy_image_hwc_rgb.shape[2] == 3: # RGB
        numpy_image_hwc_bgr = cv2.cvtColor(numpy_image_hwc_rgb, cv2.COLOR_RGB2BGR)
    else:
        raise ValueError(f"Unsupported number of channels for input image: {numpy_image_hwc_rgb.shape[2]}")

    # 2. Prepare landmarks for OpenCV TPS
    source_cv = source_landmarks_xy.reshape(-1, 1, 2).astype(np.float32)
    target_cv = target_landmarks_tps_xy.reshape(-1, 1, 2).astype(np.float32)

    num_landmarks = source_cv.shape[0]
    if num_landmarks == 0:
        return image_tensor_chw_rgb # Return original if no landmarks

    matches = [cv2.DMatch(i, i, 0) for i in range(num_landmarks)]

    # 3. Initialize and configure TPS transformer
    tps = cv2.createThinPlateSplineShapeTransformer()
    tps.setRegularizationParameter(regularization_alpha)

    # 4. Estimate transformation
    try:
        tps.estimateTransformation(target_cv, source_cv, matches)
    except cv2.error as e:
        print(f"Error during TPS estimateTransformation: {e}")
        # Handle error, e.g., by returning the original image or a black image
        return image_tensor_chw_rgb


    # 5. Warp the image
    warped_image_hwc_bgr = tps.warpImage(numpy_image_hwc_bgr)

    # 6. Convert warped image back to PyTorch tensor (CHW, RGB, original dtype and range)
    if warped_image_hwc_bgr.shape[2] == 4: # BGRA
        warped_image_hwc_rgb = cv2.cvtColor(warped_image_hwc_bgr, cv2.COLOR_BGRA2RGBA)
    else: # BGR
        warped_image_hwc_rgb = cv2.cvtColor(warped_image_hwc_bgr, cv2.COLOR_BGR2RGB)

    # Ensure contiguity before from_numpy
    warped_image_hwc_rgb_contiguous = np.ascontiguousarray(warped_image_hwc_rgb)
    warped_tensor_chw_rgb = torch.from_numpy(warped_image_hwc_rgb_contiguous).permute(2, 0, 1).to(device)

    if is_float_0_1_range:
        warped_tensor_chw_rgb = warped_tensor_chw_rgb.to(original_dtype) / 255.0
    else:
        warped_tensor_chw_rgb = warped_tensor_chw_rgb.to(original_dtype)

    return warped_tensor_chw_rgb
...
class FrameWorker(threading.Thread):
    ...
    def swap_core(self, img, kps_5, kps_all=None, s_e=None, t_e=None, parameters=None, control=None, dfm_model=False, optical_flow=None): # img = RGB
        ...
        if kps_all is not None and kps_all.shape[0] > 5 and parameters.get('UseDetailedKpsForWarpToggle', False):
            ...
            tform = trans.SimilarityTransform()
            tform.params[0:2] = M_o2c_detailed
            ...
            # Fallback to 5-point landmarks (original logic for global warp)
            print("DEBUG_SWAP_CORE: Using 5-POINT landmarks for global warp.")
        ...
        original_face_512_after_global_warp = original_face_512 # This is the input to potential TPS

        # TPS Warping (if enabled) - This refines the globally warped face
        if kps_all is not None and kps_all.shape[0] > 5 and parameters.get('UseTPSWarpingToggle', False):
            print("DEBUG_SWAP_CORE: Attempting TPS Warping")
            try:
                # Ensure tform is valid and has params before using it for TPS landmark transformation
                if tform is not None and hasattr(tform, 'params') and tform.params is not None:
                    kps_all_float = kps_all.astype(np.float32)
                    # Transform kps_all to the coordinate system of original_face_512_after_global_warp
                    kps_all_on_intermediate_warp = faceutil.trans_points2d(kps_all_float, tform.params[0:2])

                    source_landmarks_for_tps = kps_all_on_intermediate_warp
                    reference_landmarks_for_tps = kps_all_on_intermediate_warp # Refining based on its own landmarks

                    tps_alpha = parameters.get('TPSRegularizationAlphaDecimalSlider', 0.01)
                    print(f"DEBUG_SWAP_CORE: TPS landmarks count: {source_landmarks_for_tps.shape[0]}, alpha: {tps_alpha}")

                    original_face_512_after_tps = _helper_warp_image_with_tps_opencv(
                        original_face_512_after_global_warp,
                        source_landmarks_for_tps,
                        reference_landmarks_for_tps,
                        regularization_alpha=tps_alpha
                    )

                    print("DEBUG_SWAP_CORE: TPS Warping successful.")
                    original_face_512 = original_face_512_after_tps # Update original_face_512 with TPS result

                    # Re-derive lower resolution faces from the TPS-warped face
                    original_face_384 = t384(original_face_512)
                    original_face_256 = t256(original_face_512)
                    original_face_128 = t128(original_face_256)

                else:
                    print("DEBUG_SWAP_CORE: TPS Warping skipped, tform not available or invalid.")
                    # If tform is None or invalid, original_face_512 remains original_face_512_after_global_warp
                    # and derived versions are already based on it. No change needed here.

            except Exception as e_tps:
                print(f"DEBUG_SWAP_CORE: Error during TPS Warping: {e_tps}")
                # Fallback to the globally warped face if TPS fails
                original_face_512 = original_face_512_after_global_warp
                original_face_384 = t384(original_face_512)
                original_face_256 = t256(original_face_512)
                original_face_128 = t128(original_face_256)

        # Reconstruct the original_faces tuple with the correct original_face_512 (either globally warped or TPS warped)
